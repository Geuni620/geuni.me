...
이후 [#9634](https://github.com/excalidraw/excalidraw/issues/9634) Issue에서 문제가 제기되었는데, Canvas를 export 할 때 CSS filter가 결과물에 포함되지 않는 것이다.

[#2026](https://github.com/excalidraw/excalidraw/issues/2026)에서 이는 다시 다뤄졌는데, 크게 두 가지가 논의되었다.

1. `export` `preview` Canvas도 다크모드를 지원할 것인지?
2. PNG, SVG, clipboard 결과물도 다크모드를 지원할 것인지?

결과적으로 [#3046](https://github.com/excalidraw/excalidraw/pull/3046)에서 export 시 다크모드를 지원하는 PR이 추가되었다.  
해당 PR은 SVG, PNG 모두 지원했으며,

이후에 CSS filter를 제거하고 Javascript로 전환되었다. [#10578](https://github.com/excalidraw/excalidraw/pull/10578)
