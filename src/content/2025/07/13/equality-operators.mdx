...
한 가지 의문이 생겼는데, null에 대한 값 비교를 `!==` 이 아닌 [`!=`](https://github.com/toss/slash/blob/570759f2edbc9c6f413cb2ee6ab828073f61402a/packages/react/error-boundary/src/ErrorBoundary.tsx#L86)으로 하고 있는게 아닌가.

문득 머릿속에 `!=`은 뭐가 다르지? 라는 생각이 들었다.  
무분별하게 `===`, `!==`이 `==`, `!=`에 비해 더 낫다고 생각하고 무지성으로 쓰고 있었던 것이다.

함부로 `===`은 옳고, `==`은 잘못된 코드라고 판단해버린 것만 같았다. 명확한 쓰임을 모른채 말이다.
...
> 그렇기 때문에 일치 비교와 동등 비교의 차이를 알고 있어야합니다.  
> \"- You dont know JS 중 -
...
즉, error !== && error !== undefined와 동일한 의미이다.
