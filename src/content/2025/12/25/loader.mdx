...
shouldRevalidate는 해당 라우트가 재검증 대상이라고 판단되는 상황에서 호출되며,
여기서 true를 반환하면 loader가 다시 실행된다.
...
이 기능은 데이터 로딩과 무관한 전역 UI 정책(점검 모드)에 가까웠고,
React Router의 loader 재검증 정책을 변경하면서까지 구현할 필요는 없다고 판단했다.

또한 Link를 a태그로 변경하는 방법도 적절하지 않다고 생각했다.
